 ...
 * logo, nombre del proyecto, jerarquía de carpetas, colores por estado, bordes y pie de página.
 *
...
export async function generarReporteExcelPorArea(areaNombre, carpetasDelArea) {
  ...
  // --- PIE DE PÁGINA EN EL EXCEL (Filas finales) ---
  ...
  filaPie1.getCell(1).font = { italic: true, size: 8, color: { argb: "FF7F8C8D" } };
  filaPie1.getCell(1).alignment = { horizontal: "center", vertical: "middle" };

  const filaPie2 = sheet.addRow([`Fecha de emisión: ${new Date().toLocaleDateString("es-PE")} ${new Date().toLocaleTimeString("es-PE")}`]);
  sheet.mergeCells(`A${filaPie2.number}:D${filaPie2.number}`);
  filaPie2.getCell(1).font = { italic: true, size: 8, color: { argb: "FF7F8C8D" } };
  filaPie2.getCell(1).alignment = { horizontal: "center", vertical: "middle" };

  sheet.views = [{ state: "frozen", ySplit: FILA_ENCABEZADO }];

  // --- PIE DE PÁGINA NATIVO DE EXCEL (Para diseño de página e impresión) ---
